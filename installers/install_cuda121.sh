...
"$PYTHON_CMD" -m uv pip install torch>=2.1.0 --index-url https://download.pytorch.org/whl/cu121
"$PYTHON_CMD" -m uv pip install torchvision>=0.16.0 --index-url https://download.pytorch.org/whl/cu121

echo "Installing TensorRT optimization (optional, may fail if TensorRT not available)..."
if ! "$PYTHON_CMD" -m uv pip install torch-tensorrt>=2.1.0 --index-url https://download.pytorch.org/whl/cu121; then
