...
echo.
echo Installing dependencies for DirectML (AMD GPU)...
echo Project root: %PROJECT_ROOT%
echo.

echo Installing remaining dependencies...
"%PYTHON_CMD%" -m uv pip install --quiet -r "%PROJECT_ROOT%\requirements\requirements_dml.txt"
if %errorlevel% neq 0 (
    echo uv failed, falling back to pip...
    "%PYTHON_CMD%" -m pip install --quiet -r "%PROJECT_ROOT%\requirements\requirements_dml.txt"
    if %errorlevel% neq 0 exit /b %errorlevel%
)

echo.
echo [SUCCESS] Installation completed successfully!
echo Installed DirectML support for AMD GPUs. 